...
\subsection{Classifier and Fusion}
...
Training first proceeds under a file-wise stratified split, then fine-tuning uses a leakage-aware temporal split aligned with TTF: train [0,60]\%, val [60,70]\%, test [70,100.1]\%. Hyperparameters and settings for both phases are summarized in Table~\ref{tab:settings}.
...
Table~\ref{tab:abl_summary} consolidates ablations across temporal slices. Key takeaways:
\begin{itemize}
...
\begin{table}[t]
  ...
  \begin{tabular}{l l c c c c}
    ...
    \midrule
    $[70,90]\%$ & Multi-modal (vib+temp) & 0.8846 &  & 0.9388 &  \\
    $[70,90]\%$ & Classical vib (SVM, 8D) & 1.0000 &  & 1.0000 &  \\
    ...
    \midrule
    $[90,100.1]\%$ & Multi-modal (vib+temp) & 0.9231 &  &  & 0.9600 \\
    $[90,100.1]\%$ & Classical vib (SVM, 8D) & 0.0000 &  &  & 0.0000 \\
